...
import { backend, whitelist_frontend } from "../../src/config/envvars";
...
describe("dbConnection", () => {
  ...
  it("should throw an error if backend URL is not provided", async () => {
    ...
    jest
      .spyOn(require("../../src/config/setup"), "backend", "get")
    ...
    jest
      .spyOn(require("../../src/config/setup"), "backend", "get")
      .mockReturnValue(originalBackend);
  });
  ...
  it("should attempt to connect to the database using the provided backend URL", async () => {
    ...
    jest
      .spyOn(require("../../src/config/setup"), "backend", "get")
      .mockReturnValue(testBackendUrl);
