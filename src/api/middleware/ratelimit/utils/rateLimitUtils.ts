...
export class HeaderUtils {
  ...
  static addRateLimitHeaders(
    ...
    const headers = this.generateRateLimitHeaders(limit, remaining, resetTime, retryAfter);
  ...
  static extractRateLimitInfo(headers: Record<string, string>): {
    ...
    return {
      ...
      reset: headers['x-ratelimit-reset'] ? new Date(headers['x-ratelimit-reset']).getTime() : undefined,
...
export class RateLimitUtils {
   ...
   */
  static shouldApplyRateLimit(req: Request, rateLimitType: RateLimitType): boolean {
   ...
   */
  static getRateLimitKey(req: Request, rateLimitType: RateLimitType, accessLevel?: AccessLevel): string {
  ...
  static createRateLimitError(
    ...
    switch (rateLimitType) {
      case RateLimitType.GLOBAL:
        return RateLimitErrorDTO.createGlobalError(retryAfter, remainingRequests, resetTime);
      
      case RateLimitType.LOGIN:
        return RateLimitErrorDTO.createLoginError(retryAfter, remainingRequests, resetTime);
      
      case RateLimitType.SIGNUP:
        return RateLimitErrorDTO.createSignUpError(retryAfter, remainingRequests, resetTime);
      
      case RateLimitType.API:
        return RateLimitErrorDTO.createApiError(retryAfter, remainingRequests, resetTime);
      
      case RateLimitType.USER:
        return RateLimitErrorDTO.createUserError(retryAfter, accessLevel || AccessLevel.USER, remainingRequests, resetTime);
      
      case RateLimitType.SUPERVISOR:
        return RateLimitErrorDTO.createSupervisorError(retryAfter, remainingRequests, resetTime);
      
      case RateLimitType.ADMIN:
        return RateLimitErrorDTO.createAdminError(retryAfter, remainingRequests, resetTime);
...
export { IPUtils, HeaderUtils, RateLimitUtils };
